...
@CrewBase
class NewsScrapperAgent():
	"""NewsScrapperAgent crew"""
	...
	@agent
	def get_news(self) -> Agent:
		return Agent(
			config=self.agents_config['get_news'],
			tools=[SerperDevTool()],
			verbose=True
		)
	...
	@agent
	def ai_news(self) -> Agent:
		return Agent(
			config=self.agents_config['ai_news'],
			tools=[],
			verbose=True
		)
	@agent
	...
	@task
	def ai_news_write(self) -> Task:
		return Task(
			config=self.tasks_config['ai_news_write'],
		)
	...
	@crew
	def crew(self) -> Crew:
		"""Creates the NewsScrapperAgent crew"""
